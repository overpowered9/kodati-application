...
const DeleteRegion = ({ regionId, onClose }: { regionId: number, onClose: () => void }) => {
  ...
  const handleDelete = async () => {
    ...
    if (!response.ok) {
      ...
      throw new Error(data?.error || 'An error occurred while deleting the region');
    }
  };

  const { mutate: deleteRegion, isPending: loading } = useMutation({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
      <div className="bg-white p-4 rounded-md">
        <h2 className="text-2xl font-semibold mb-2">Delete Region</h2>
